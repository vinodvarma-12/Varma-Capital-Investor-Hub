    ...
    <div className="min-h-screen bg-gray-950 p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="space-y-2">
          ...
          <p className="text-gray-400">Manage all investor support tickets</p>
        </div>

        <div className="grid lg:grid-cols-3 gap-6 h-[calc(100vh-12rem)]">
          {/* Ticket List */}
          <Card className="lg:col-span-1 bg-gray-900 border-gray-800 flex flex-col">
            <CardHeader>
              ...
              <div className="flex gap-2 mt-4">
                <Select value={filters.status} onValueChange={(val) => setFilters(f => ({...f, status: val}))}>
                  ...
                  <SelectContent className="bg-gray-800 text-white"><SelectItem value="all">All Statuses</SelectItem><SelectItem value="open">Open</SelectItem><SelectItem value="in_progress">In Progress</SelectItem><SelectItem value="resolved">Resolved</SelectItem><SelectItem value="closed">Closed</SelectItem></SelectContent>
                </Select>
                <Select value={filters.priority} onValueChange={(val) => setFilters(f => ({...f, priority: val}))}>
                  <SelectTrigger className="bg-gray-800 border-gray-700"><SelectValue/></SelectTrigger>
                  <SelectContent className="bg-gray-800 text-white"><SelectItem value="all">All Priorities</SelectItem><SelectItem value="low">Low</SelectItem><SelectItem value="medium">Medium</SelectItem><SelectItem value="high">High</SelectItem><SelectItem value="urgent">Urgent</SelectItem></SelectContent>
                </Select>
              </div>
            </CardHeader>
              ...
              <div className="space-y-3">
                {filteredTickets.map(ticket => (
                  <div key={ticket.id} onClick={() => setSelectedTicket(ticket)}
                    className={`p-3 rounded-lg cursor-pointer transition-colors ${selectedTicket?.id === ticket.id ? 'bg-gray-800' : 'hover:bg-gray-800/50'}`}>
                    ...
                    <p className="text-sm text-gray-400 truncate">{getUserName(ticket.investor_email)}</p>
                    <div className="flex justify-between items-center mt-2">
                       <Badge variant="outline" className={`capitalize text-xs ${getStatusBadge(ticket.status)}`}>{ticket.status.replace('_', ' ')}</Badge>
                       <p className="text-xs text-gray-500">{formatDistanceToNow(new Date(ticket.updated_date), { addSuffix: true })}</p>
                    </div>
                  </div>
                ))}
                {filteredTickets.length === 0 && <p className="text-center text-gray-500 py-8">No tickets match filters.</p>}
              </div>
            </CardContent>
          </Card>
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                      <Select value={selectedTicket.status} onValueChange={(val) => handleUpdateTicket(selectedTicket.id, 'status', val)}>
                         <SelectTrigger className="w-[140px] bg-gray-800 border-gray-700"><SelectValue/></SelectTrigger>
                         <SelectContent className="bg-gray-800 text-white"><SelectItem value="open">Open</SelectItem><SelectItem value="in_progress">In Progress</SelectItem><SelectItem value="resolved">Resolved</SelectItem><SelectItem value="closed">Closed</SelectItem></SelectContent>
                      </Select>
                       <Select value={selectedTicket.priority} onValueChange={(val) => handleUpdateTicket(selectedTicket.id, 'priority', val)}>
                         ...
                         <SelectContent className="bg-gray-800 text-white"><SelectItem value="low">Low</SelectItem><SelectItem value="medium">Medium</SelectItem><SelectItem value="high">High</SelectItem><SelectItem value="urgent">Urgent</SelectItem></SelectContent>
                      </Select>
                    </div>
                  </div>
                  <p className="text-sm text-gray-400">From: {getUserName(selectedTicket.investor_email)}</p>
                </CardHeader>
                <CardContent className="flex-1 space-y-4 overflow-y-auto p-4">
                  <div className="bg-gray-800/50 p-4 rounded-lg">
                    <p className="text-sm font-semibold text-gray-300 mb-2">Initial Description</p>
                  ...
                  {messages.map(msg => (
                    <div key={msg.id} className={`flex gap-3 ${msg.sender_email === currentUser.email ? 'justify-end' : 'justify-start'}`}>
                      <div className={`p-3 rounded-lg max-w-lg ${msg.sender_email === currentUser.email ? 'bg-yellow-900/80 text-yellow-100' : 'bg-gray-800 text-gray-300'}`}>
                ...
                <CardFooter className="p-4 border-t border-gray-800">
                  <div className="relative w-full">
                    <Textarea 
                      ...
                      rows={3}
                    />
                    <Button size="icon" onClick={handleSendMessage} className="absolute top-1/2 right-2 transform -translate-y-1/2 bg-yellow-400 hover:bg-yellow-500">
