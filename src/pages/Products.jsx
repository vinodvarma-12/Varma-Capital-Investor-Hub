...
export default function Products() {
  ...
  const loadData = async () => {
    try {
      ...
      const visibleProducts = allProductsData.filter(
        (p) => p.status === "active",
      );
  ...
  const handleAllocationRequest = async () => {
    if (
      !selectedProduct ||
      !requestAmount ||
      parseFloat(requestAmount) < selectedProduct.minimum_ticket
    ) {
      return;
    }
    ...
    try {
      ...
      // Notify super admins — fire-and-forget, don't block the UX
      console.log("[AllocationNotify] Invoking edge function with:", {
        productName: selectedProduct.name,
        requestedAmount: parseFloat(requestAmount),
        productId: selectedProduct.id,
      });
      invokeEdgeFunction("notify-allocation-request", {
        productName: selectedProduct.name,
        requestedAmount: parseFloat(requestAmount),
        productId: selectedProduct.id,
      })
        .then((res) => {
          console.log("[AllocationNotify] Success:", res);
        })
        .catch((err) => {
          console.error("[AllocationNotify] Failed:", err);
        });

      setPendingProductIds((prev) => new Set([...prev, selectedProduct.id]));
      closeDialog();
      alert(
        "Allocation request submitted successfully! You will be notified once reviewed.",
      );
    } catch (error) {
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {products.map((product) => (
            <Card
              key={product.id}
              className={`bg-card border border-[#ccab6c]/30 relative overflow-hidden ${product.image_url ? "pt-0" : ""} ${!product.is_public ? "opacity-80" : ""}`}
            >
              {/* Product image */}
              {product.image_url && (
                <div
                ...
                <div className="flex items-start justify-between">
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      {!product.is_public && (
                        <Badge className="bg-blue-900/60 text-blue-300 border-blue-700/60 text-xs">
                          Coming Soon
                        </Badge>
                      )}
                    </div>
              ...
              <CardContent className="space-y-6">
                ...
                <div className="space-y-2">
                  ...
                  {(product.redemption_penalty_percent > 0 ||
                    product.redemption_penalty_amount > 0) && (
                    <div className="flex items-center gap-2 text-sm text-gold/90">
                ...
                {!product.is_public ? (
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-center text-gold/70">
                      Your request is under review. This button will be enabled
                      once the authority takes action.
                    </p>
                  </div>
                ) : (
                  <Button
          ...
          <div className="space-y-6 py-4">
            <div className="space-y-2">
              ...
              {selectedProduct && (
                <p className="text-xs text-gold/90">
                  Minimum investment: $
                  {selectedProduct.minimum_ticket?.toLocaleString()}
                </p>
              )}
            </div>
            ...
            <div className="bg-muted rounded-lg p-4 space-y-2">
              ...
              <ul className="text-sm text-foreground/80 space-y-1">
                ...
                <li>
                  • Funds will be locked for {selectedProduct?.lock_in_months}{" "}
                  months
                </li>
                <li>
                  • Management fee: {selectedProduct?.management_fee_percent}%
                  per annum
                </li>
              ...
              <Button
                ...
                disabled={
                  ...
                  parseFloat(requestAmount) <
                    (selectedProduct?.minimum_ticket || 0)
                }
              >
